...
void LineFollower::followLine() {
    ...
    if (position == -999) {
        ...
        if (lastPosition != -999 && timeSinceLine < LINE_MEMORY_TIMEOUT) {
            ...
            // Используем историю позиций для предсказания направления!
            // ═══════════════════════════════════════════════════════════════
            
            // Получаем тренд (куда двигалась линия)
            float trend = sensors.getPositionTrend();
            float avgPosition = sensors.getAveragePosition();
            
            // Комбинированная оценка направления:
            // - lastPosition: где линия была последний раз
            // - trend: куда она двигалась
            // - avgPosition: средняя позиция (для фильтрации шума)
            float predictedDirection = lastPosition + trend * 0.5;
            
            // Резкий поворот? ОСТАНАВЛИВАЕМСЯ и ждём обновления энкодеров!
            // Требуем сильное отклонение (2.5+) И потерю линии более 100мс
            if (abs(lastPosition) >= 2.5 && timeSinceLine >= 100) {
                ...
                turnDirection = (predictedDirection < 0) ? TURN_LEFT : TURN_RIGHT;
...
#ifdef DEBUG_MODE
                Serial.printf("[%lu] ⏸ СТОП перед поворотом (поз=%.1f тренд=%.2f) → ждём 200мс\n",
                              millis(), lastPosition, trend);
#endif
                return;
            }
            ...
            // Используем предсказанное направление для поиска
            int leftSpeed, rightSpeed;
            
            if (predictedDirection > 0) {
                // Линия уходит вправо - крутим вправо
                leftSpeed = searchSpeed;
                rightSpeed = -MIN_SPEED;
            } else {
                // Линия уходит влево - крутим влево
                leftSpeed = -MIN_SPEED;
            ...
            if (millis() - lastLostDebug > 200) {
                Serial.printf("[%lu] 🔍 ПОИСК (поз=%.1f тренд=%.2f предск=%.2f %lu мс) | M: L=%d R=%d\n",
                              millis(), lastPosition, trend, predictedDirection, timeSinceLine, leftSpeed, rightSpeed);
