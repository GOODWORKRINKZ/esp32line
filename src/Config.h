...
// ВНИМАНИЕ: При автономной работе (без USB) DEBUG_MODE замедляет робота!
// Включайте только при подключении к компьютеру для отладки
//#define DEBUG_MODE
...
// Бузер
#define BUZZER_PIN     33  // Пин бузера
#define BUZZER_CHANNEL 4   // LEDC канал для бузера (0-3 заняты моторами)
...
// Калибровочный множитель мощности моторов (0.0-1.0)
// Установлен в 1.0 после настройки BASE_SPEED и PID под реальную мощность
#define MOTOR_POWER_SCALE 0.71

#define BASE_SPEED     130  // Базовая скорость (0-255) - откалибровано под моторы
#define MAX_SPEED      200  // Максимальная скорость
#define MIN_SPEED      80   // Минимальная скорость (выше мёртвой зоны моторов!)
#define TURN_SPEED     130  // Скорость при повороте на месте

// ═══════════════════════════════════════════════════════════════════════════
// ПАРАМЕТРЫ ПИД
// ═══════════════════════════════════════════════════════════════════════════

// Базовый ПИД - снижены коэффициенты под BASE_SPEED=130 (вместо прежних 160)
#define DEFAULT_KP     16.0  // Пропорциональный (было 20.0 для скорости 160)
#define DEFAULT_KI     0.0   // Интегральный коэффициент
#define DEFAULT_KD     10.0  // Дифференциальный (было 12.0)

// Агрессивный PID (при большой ошибке) - также снижены пропорционально
#define AGGRESSIVE_KP  32.0   // Kp для резких поворотов (было 40.0)
#define AGGRESSIVE_KD  20.0   // Kd для резких поворотов (было 25.0)
...
#define LINE_MEMORY_TIMEOUT  500  // Время памяти последней позиции линии (мс) - увеличено для широкой линии
#define OVERSHOOT_DETECT_TIME 100  // Минимальное время потери линии для детекции overshoot (мс)
#define POSITION_HISTORY_SIZE 7   // Размер истории позиций для предсказания направления
